...
model <-lm(sentence ~ sexdiff*attract, data=df,
          contrasts=list(sexdiff=contr.sum, attracte=contr.sum), type = 3)
car::Anova(model, type=3)

```


From the output you can see that even though `Anova()` requires more complex input it is more useful as it also returns the residuals, `anova_test()` does not do that. In  your future work you **should only use aov() when your data are balanced** in any other case you should use either `anova_test()` or `car::Anova()`. For peace of mind you can use the two latter ones in all cases. 
...
## we then use the emmeans function
## we declare the model we use, and the variables for the simple main effects
...
## We can also just request an output of the above two emmeans
## this will give us the marginal means wtih confidence intervals
...
The `emmeans` is a very powerful package that provides a plethora of tools for our analysis. For example, we can have a quick visual display of our interaction, see below. There are further uses of `emmeans` and you will learn more about them in during your Practical B.




```{r emmeans.emmip, echo=TRUE, message=FALSE, warning=FALSE}
## We use the same arguments from our simple main effect exploration
## this time with the function emmip (ip= interaction plot)

emmip(model1,  ~ sexdiff | attract)

emmip(model1,  ~ attract | sexdiff)
...
**Simple main effect of degree in the male condition: F(4,1331) = 22.4, p < .001**

**Simple main effect of degree in the female condition: F(4,1331) = 46.9, p < .001**

**Simple main effect of sex in the lower than high school condition: F(1,1331) = 22.156, MSE = 15.773, p < .001

**Simple main effect of sex in the high school condition: F(1,1331) = .289, MSE = 15.773, p = .591

**Simple main effect of sex in the junior college condition: F(1,1331) = .222, MSE = 15.773, p = .638

**Simple main effect of sex in the bachelor condition: F(1,1331) = 2.727, MSE = 15.773, p = .099

**Simple main effect of sex in the graduate condition: F(1,1331) = .082, MSE = 15.773, p = .775
